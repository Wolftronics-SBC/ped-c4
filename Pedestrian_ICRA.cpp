...
// Load SVM models -- Histogram Intersection Kernel SVM trained by libHIK
...
int DetectHuman(const char* filename,const char* outname,DetectionScanner& ds,std::ofstream& out)
{
	...
	for(unsigned int i=0;i<results.size();i++)
		out<<results[i].score<<' '<<results[i].left<<' '<<results[i].top<<' '<<results[i].right-results[i].left<<' '<<results[i].bottom-results[i].top<<std::endl;
    ...
    if(outname)
    {
        ...
        for(unsigned int i=0;i<results.size();i++)
        {
            ...
            w = std::min((results[i].right-results[i].left) * 1.35, img.cols - results[i].left);
            h = std::min((results[i].bottom-results[i].top) * 1.2, img.rows - results[i].top);
            x = std::max(.0, results[i].left - (w - (results[i].right-results[i].left))/2);
            y = std::max(.0, results[i].top - (h - (results[i].bottom-results[i].top))/2);


            cv::Rect myROI(x, y, w, h); // (x,y,w,h)
            cv::Mat croppedRef(img, myROI);
            my_itoa(i+1, id, 10);
            cv::imwrite(fn.substr(0, p+1) + id + ".jpg", croppedRef);
        }
